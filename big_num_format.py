...
def get_name(magnitude_over_3, shorten=False):
    ...
    if not shorten:
        ...
        for big_units, tens in [(names_lines[2].split(",")[i],
                                 names_lines[3].split(",")[i]) for i in range(9)]:
            big_units_append = ""
            tens_append = ""
    ...
    if magnitude_over_3 < 11:
        ...
        if not shorten:
            ...
            exception_char = "".join(set(big_units_flags[magnitude_units_digit-1])
...
def format_num(number, shorten=False, precision=0:
    if abs(number) >= 1e100:
        raise ValueError("Number is bigger than or equal to 1e100.")
